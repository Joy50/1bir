...
SLOT_LEAVE_TYPE_NAMES = {
    ...
    LeaveState.SLOT_C_LEAVE_2: "Casual Leave",
    LeaveState.SLOT_C_LEAVE_3: "Casual Leave",
    LeaveState.SLOT_C_LEAVE_4: "Casual Leave",
    LeaveState.SLOT_C_LEAVE_5: "Casual Leave",
    LeaveState.SLOT_J_LEAVE: "Joining Leave",
    LeaveState.SLOT_M_LEAVE: "Medical Leave",
    LeaveState.SLOT_COURSE: "Course",
    LeaveState.SLOT_CADRE: "Cadre",
    LeaveState.SLOT_COMD: "Command",
    LeaveState.SLOT_ATT: "Attachment",
    LeaveState.SLOT_HOSP: "Hospital",
    LeaveState.SLOT_DEMOB: "Demobilization",
    LeaveState.SLOT_FDMN: "Field Mission",
    LeaveState.SLOT_TEKNAF: "Teknaf",
    LeaveState.SLOT_OSL: "OSL",
}


def leave_type_for_slot(slot):
    if slot.startswith("C lve"):
        name = "Casual Leave"
    else:
...
class LeaveApplyForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        other_slots = [
            (key, label)
            for key, label in LeaveState.SLOT_CHOICES
            if not LeaveState.is_casual_slot(key)
        ]
        casual_slots = (
            LeaveState.available_casual_slots(self.soldier, year)
            if self.soldier
            else [(LeaveState.SLOT_C_LEAVE_1, LeaveState.SLOT_C_LEAVE_1)]
        )
        clerk_only = bool(self.applicant and self.applicant.is_clerk)
        if clerk_only:
            slot_choices = [
                (key, label)
                for key, label in other_slots
                if key == LeaveState.SLOT_P_LEAVE
            ] + casual_slots
            help_text = "Company clerks can apply for P Leave and C Leave. Any officer of this company or the CO can approve it on Leave State."
        else:
            slot_choices = other_slots[:1] + casual_slots + other_slots[1:]
            help_text = ""
    ...
    def clean(self):
        ...
        if slot:
            ...
            if (
                ...
                and slot
                and slot != LeaveState.SLOT_P_LEAVE
                and not LeaveState.is_casual_slot(slot)
            ):
                self.add_error(
                    "slot",
